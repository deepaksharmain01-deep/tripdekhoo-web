...
const Leveltwopages = () => {
  ...
  const { destination, subdestination, id } = useParams();
  
  // Synchronous lookup for immediate SEO updates
  const destinationObj = allPackagesData.destinations.find(
    (dest) => dest.destination.toLowerCase().trim() === destination.toLowerCase().trim()
  );

  const subdestinationObj = destinationObj?.packages?.find(
    (subdest) => subdest.destination.toLowerCase().trim() === subdestination.toLowerCase().trim()
  );

  const matchedPackage = subdestinationObj?.packages?.find(
    (pkg) => pkg.id?.toLowerCase().trim() === id?.toLowerCase().trim()
  );

  const currentPolicies = matchedPackage?.international ? policies.international : policies.domestic;
  ...
  return (
    <div className="w-100" style={{ paddingTop: 'var(--navbar-height, 80px)' }}>
      <SEO 
        key={`${destination}-${subdestination}-${id}`}
        title={matchedPackage?.title || (id ? id.replace(/-/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : (subdestination || destination))}
        description={matchedPackage?.subTitle}
        image={matchedPackage?.image}
        url={`/packages/${destination}/${subdestination}/${id}`}
      />
      {matchedPackage === null ? (
        ...
        <div>
          <div id="destinationCarousel" className="carousel slide" data-bs-ride="carousel">
            <div className="carousel-inner" style={{overflow:"visible"}}>
              {[matchedPackage.image, matchedPackage.image2, matchedPackage.image3].map((img, index) => (
                <div className={`carousel-item ${index === 0 ? "active" : ""}`} key={index}>
                  <div className="image-container position-relative">
                    <img className="w-100" src={img} alt={`Slide ${index + 1}`} />
                    <div
                    ...
                    <div className="details-text-overlap text-center text-white" style={{ zIndex: 2 }}>
                      <p className="mb-1">
                        Get <span className="text-warning fw-bold">{matchedPackage?.discount}% OFF</span> on
                      </p>
                      <h1 className="fw-bold">{matchedPackage?.title}</h1>
                      <p>{matchedPackage?.subTitle}</p>
                      <h3 className="fw-bold text-warning">{matchedPackage.name}</h3>
                      <p>
                        Starting at{" "}
                    ...
                    <div className="connect-btn-wrapper text-center position-absolute" style={{zIndex:"2"}} onClick={openModal}>
                      <button
                        className="btn d-flex align-items-center text-white fw-bold px-4 py-2 me-3"
                        style={{ backgroundColor: "#7B5CFF", borderRadius: "50px", flex: 1 }}
                      >
                        <img src={phone} alt="phone" className="me-2 bg-white text-dark rounded-circle p-2" style={{ width: "40px", height: "40px" }} />
              ...
              <div className="row justify-content-center">
                <div className="col-12 col-xl-10">
                  ...
                  <div className="col-12 mt-5">
                    <div className="itinerary-section">
                      <div className="text-center mb-5">
                        ...
                        <div className="itinerary-floating-controls"> 
                          <button className="floating-btns expand-btns" onClick={() => setExpandedDays(matchedPackage.itinerary.map((_, i) => i))} > 
                            <svg width="22" height="22" fill="white" viewBox="0 0 24 24"> <path d="M12 5V19M5 12H19" stroke="white" strokeWidth="2" strokeLinecap="round"/> </svg> 
                          </button> 
                          <button className="floating-btns collapse-btns" onClick={() => setExpandedDays([])} > 
                            <svg width="22" height="22" fill="white" viewBox="0 0 24 24"> <path d="M5 12H19" stroke="white" strokeWidth="2" strokeLinecap="round"/> </svg> 
                          </button> 
                        </div>
                      </div>
                      <div className="itinerary-timeline">
                        {matchedPackage.itinerary && matchedPackage.itinerary.map((day, index) => (
                          <div key={`day-${index}`} className="itinerary-day-card">
                            <div className="day-number-badge"><span className="day-number">{index + 1}</span></div>
                            <div className="day-content-card">
                              <div className="day-header" onClick={() => toggleDay(index)}>
                                <div className="day-info">
                                  <h3 className="day-title">{day.day}</h3>
                                  <span className="activities-count">{day?.activities?.length || 0} Activities</span>
                                </div>
                                <div className={`expand-icon ${expandedDays.includes(index) ? 'expanded' : ''}`}>
                              ...
                              <div className={`day-content ${expandedDays.includes(index) ? 'expanded' : ''}`}>
                                {day?.images && day.images.length > 0 && (
                                  <details className="accordion-section p-3" open>
                                    <summary className="accordion-header activities-title d-flex align-items-center gap-2" style={{ fontSize: "18px" }}>
                                      <Camera size={22} /> Explore Photo Moments
                                    </summary>
                                    <Swiper modules={[Navigation, Pagination, Autoplay]} spaceBetween={10} slidesPerView={1} navigation loop={true} autoplay={{ delay: 3000, disableOnInteraction: false }} speed={800}>
                                      {day.images.map((img, idx) => (
                                        <SwiperSlide key={idx} style={{borderRadius:"10px"}}>
                                          <div className="swiper-image-container" style={{borderRadius:"10px"}} >
                                            <img src={img} alt={`${day.day} - ${idx + 1}`} className="day-image" style={{borderRadius:"10px"}} />
                                            <div className="image-overlay" style={{borderRadius:"10px"}}></div>
                                            <div className="image-caption" style={{borderRadius:"10px"}}><span className="caption-text">{day.day}</span></div>
                                          </div>
                                        </SwiperSlide>
                                      ))}
                                    </Swiper>
                            ...
                            {index < matchedPackage.itinerary.length - 1 && <div className="timeline-connector"></div>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="container text-center my-5">
                    <h4 className="fw-bold" style={{ color: "#4B47F5" }}>Prefer to discuss?</h4>
                    <p className="text-muted mb-4">Choose an option below as per your convenience</p>
                    <div className="d-flex justify-content-center">
                      <div className="d-flex flex-wrap justify-content-center align-items-center border rounded-pill p-3 shadow-sm bg-white" style={{ maxWidth: "650px" }}>
                        <button
                          ...
                          className="btn d-flex align-items-center text-white fw-bold px-4 py-3 flex-grow-1"
                          style={{ backgroundColor: "#25D366", borderRadius: "50px", minWidth: "250px" }}
                        >
                          <img src={whatsapp} alt="WhatsApp" className="me-3 bg-white rounded-circle p-2" style={{ width: "40px", height: "40px" }} />
                          <div className="text-start lh-sm"><div>WhatsApp</div><small className="fw-normal">Send your Enquiry</small></div>
                        </button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h3 className="text-center fw-bold" style={{ color: "#4B47F5" }}>What inside the package?</h3>
                  ...
                  <div className="package-tabs-container">
                    ...
                    <div className="tab-content">
                      <div className="tab-content-header">
                        <h3 className="content-title">{tabs.find(tab => tab.key === activeTab)?.label} Details</h3>
                        <span className="content-count">{tabs.find(tab => tab.key === activeTab)?.data.length || 0} items</span>
                      </div>
                      ...
                      {tabs.find(tab => tab.key === activeTab)?.data.length > 5 && (
                        <div className="show-more-container">
                          <button className="show-more-button" onClick={() => setShowAllContent(!showAllContent)}>
                            <span>{showAllContent ? "Show Less" : "Show More"}</span>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className={`arrow-icon ${showAllContent ? 'rotated' : ''}`}>
                              <path d="M6 9L12 15L18 9" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                          </button>
                        </div>
                      ...
                      <div className="col-12">
                        {Object.entries(currentPolicies).map(([policyName, policyItems]) => (
                          <div key={policyName} className="card mb-3 shadow-sm border-0">
                            <div className="card-header bg-white border-0 d-flex justify-content-between align-items-center" style={{ cursor: 'pointer', padding: '1rem 1.5rem' }} onClick={() => toggleSection(policyName)}>
                              <h5 className="mb-0 fw-bold" style={{ color: '#333' }}>{policyName} – <span style={{ color: '#6366f1' }}>TripDekhoo</span></h5>
                              <i className={`fas ${expandedSections[policyName] ? 'fa-chevron-up' : 'fa-chevron-down'}`} style={{ color: '#6c757d', fontSize: '14px' }}></i>
                            </div>
                                ...
                                <ul className="list-unstyled mb-0">
                                  {policyItems.map((item, index) => (
                                    <li key={index} className="mb-3 d-flex">
                                      <span className="me-3 flex-shrink-0" style={{ color: '#6366f1', fontSize: '14px', marginTop: '2px', fontWeight: 'bold' }}>•</span>
                                      <span style={{ color: '#495057', lineHeight: '1.6' }}>{item}</span>
                                    </li>
                                  ))}
                                </ul>
                              </div>
                        ...
                        <div className="text-center mt-4">
                          <p className="text-muted small mb-0">
                            Showing {matchedPackage.international ? 'International' : 'Domestic'} Travel Policies
                          </p>
                        </div>
                      </div>
                    </div>
